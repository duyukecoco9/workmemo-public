...
在 Render 的环境变量中维护 `WORKMEMO_ACCESS_CODES`，值为 JSON 对象：

```json
{"workmemo":"default","team-a":"team-a","team-b":"team-b"}
```

左侧是登录代码，右侧是数据空间名。不同空间会写入不同的数据文件和历史快照。修改环境变量后重新部署即可生效；不要把真实代码提交到公开仓库。默认未配置时仅启用 `workmemo → default`。

登录成功后，前端会自动使用该代码对应的数据空间；同一代码在不同设备登录后读取同一份服务器数据。
